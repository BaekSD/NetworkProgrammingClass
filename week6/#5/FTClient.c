...
void FTClient(int sock, struct sockaddr_in serv_addr) {
	...
	while(1) {
		...
		if(input == 'p') {
			...
			if(sendto(sock, filename, FILENAME_LEN-1, 0, (struct sockaddr*)&serv_addr, sizeof(serv_addr)) != FILENAME_LEN-1) {
				DieWithError("send() sent a different number of bytes than expected");
			}
			if(sendto(sock, str_filesize, 31, 0, (struct sockaddr*)&serv_addr, sizeof(serv_addr)) != 31) {
				DieWithError("send() sent a different number of bytes than expected");
			}
			...
			while(totalByteSent < file_size) {
				...
				totalByteSent += fsize;
				if(sendto(sock, buf, fsize, 0, (struct sockaddr*)&serv_addr, sizeof(serv_addr)) != fsize) {
					DieWithError("send() sent a different number of bytes than expected");
				}
			...
			if(msgType == FileNak) {
				...
				if((bytesRcvd = recvfrom(sock, str_filesize, 31, 0, (struct sockaddr*)&from_addr, &addr_size)) < 0) {
					DieWithError("recv() failed or connection closed prematurely");
				}
				...
				while(totalByteRcvd < file_size) {
					if((bytesRcvd = recvfrom(sock, buf, PACKET_SIZE, 0, (struct sockaddr*)&from_addr, &addr_size)) < 0) {
						DieWithError("recv() failed or connection closed prematurely");
					}
			...
			if((bytesRcvd = recvfrom(sock, &msgType, sizeof(char), 0, (struct sockaddr*)&from_addr, &addr_size)) != sizeof(char)) {
				DieWithError("recv() failed or connection closed prematurely");
			}
			...
			if(msgType == LSNak) {
				...
				for(i=0; i<file_size; i++) {
					if((bytesRcvd = recvfrom(sock, filename, FILENAME_LEN-1, 0, (struct sockaddr*)&from_addr, &addr_size)) < 0) {
						DieWithError("recv() failed or connection closed prematurely");
					}
