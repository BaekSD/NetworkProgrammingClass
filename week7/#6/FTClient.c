...
void FTClient(int sock, int ftsock) {
	...
	while(1) {
		...
		if(input == 'p') {
			...
			if(send(ftsock, filename, FILENAME_LEN-1, 0) != FILENAME_LEN-1) {
				DieWithError("send() sent a different number of bytes than expected");
			}
			...
			if((bytesRcvd = recv(ftsock, &msgType, sizeof(char), 0)) < 0) {
			...
			if(msgType == FileNak) {
				...
				if((bytesRcvd = recv(ftsock, str_filesize, 31, 0)) < 0) {
					DieWithError("recv() failed or connection closed prematurely");
				}
				...
				while(totalByteRcvd < file_size) {
					if((bytesRcvd = recv(ftsock, buf, PACKET_SIZE, 0)) < 0) {
						DieWithError("recv() failed or connection closed prematurely");
					}
			...
			if((bytesRcvd = recv(ftsock, &msgType, sizeof(char), 0)) != sizeof(char)) {
				DieWithError("recv() failed or connection closed prematurely");
			}
			...
			if(msgType == LSNak) {
				...
				for(i=0; i<file_size; i++) {
					if((bytesRcvd = recv(ftsock, filename, FILENAME_LEN-1, 0)) < 0) {
						DieWithError("recv() failed or connection closed prematurely");
					}
